...
#include "vcu_planning_simulator/vehicle_model/sim_model.hpp"
...
VcuPlanningSimulator::VcuPlanningSimulator(const rclcpp::NodeOptions & options)
: Node("vcu_planning_simulator", options), tf_buffer_(get_clock()), tf_listener_(tf_buffer_)
{
  ...
  sub_gear_report_ = create_subscription<GearReport>(
    "input/gear_report", QoS{1},
    [this](const GearReport::SharedPtr msg) { current_gear_report_ = *msg; });
  ...
  sub_steer_report_ = create_subscription<SteeringReport>(
    "input/steer_report", QoS{1},
    [this](const SteeringReport::SharedPtr msg) { current_steer_report_ = *msg; });
  sub_velocity_report_ = create_subscription<VelocityReport>(
    "input/velocity_report", QoS{1},
    [this](const VelocityReport::SharedPtr msg) { current_velocity_report_ = *msg; });
  sub_trajectory_ = create_subscription<Trajectory>(
    "input/trajectory", QoS{1}, std::bind(&VcuPlanningSimulator::on_trajectory, this, _1));

  srv_mode_req_ = create_service<ControlModeCommand>(
    "input/control_mode_request",
    std::bind(&VcuPlanningSimulator::on_control_mode_request, this, _1, _2));
  ...
  set_param_res_ = this->add_on_set_parameters_callback(
    std::bind(&VcuPlanningSimulator::on_parameter, this, _1));
  ...
  on_timer_ = rclcpp::create_timer(
    ...
    std::bind(&VcuPlanningSimulator::on_timer, this));
  ...
  srv_set_pose_ = proxy.create_service<tier4_external_api_msgs::srv::InitializePose>(
    "/api/simulator/set/pose", std::bind(&VcuPlanningSimulator::on_set_pose, this, _1, _2),
...
void VcuPlanningSimulator::initialize_vehicle_model()
{
...
rcl_interfaces::msg::SetParametersResult VcuPlanningSimulator::on_parameter(
...
void VcuPlanningSimulator::on_timer()
{
  if (!is_initialized_) {
  ...
  // update vehicle dynamics
  {
    ...
    if (current_control_mode_.mode == ControlModeReport::AUTONOMOUS) {
      vehicle_model_ptr_->setGear(current_gear_report_.report);
      set_input_from_vehicle_report(current_steer_report_, current_velocity_report_);
    } else {
...
void VcuPlanningSimulator::on_set_pose(
...
void VcuPlanningSimulator::set_input(const AckermannControlCommand & cmd)
{
...
void VcuPlanningSimulator::set_input_from_vehicle_report(const SteeringReport & steering_report, const VelocityReport & velocity_report)
{
  const auto steer = steering_report.steering_tire_angle;
  const auto vel = velocity_report.longitudinal_velocity;
...
void VcuPlanningSimulator::on_trajectory(const Trajectory::ConstSharedPtr msg)
{
  current_trajectory_ptr_ = msg;
}

void VcuPlanningSimulator::on_engage(const Engage::ConstSharedPtr msg)
{
  simulate_motion_ = msg->engage;
}

void VcuPlanningSimulator::on_control_mode_request(
...
void VcuPlanningSimulator::add_measurement_noise(
...
void VcuPlanningSimulator::set_initial_state_with_transform(
...
void VcuPlanningSimulator::set_initial_state(const Pose & pose, const Twist & twist)
{
...
double VcuPlanningSimulator::get_z_pose_from_trajectory(const double x, const double y)
{
...
TransformStamped VcuPlanningSimulator::get_transform_msg(
...
void VcuPlanningSimulator::publish_odometry(const Odometry & odometry)
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(simulation::vcu_planning_simulator::VcuPlanningSimulator)
